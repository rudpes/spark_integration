...
def random_forest_implementation(slice_name, n_trees, depth, spark):
    global model, predictions, accuracy, f1, weightedPrecision, weightedRecall
    minimal = spark.sql("SELECT * FROM " + slice_name)
    drop_cols = ['player_slot', 'matchId']
    minimal = minimal.drop(*drop_cols)
    core_cols = ['time']
    hero_cols = ['hero_name']
    main_cols = constants.main_columns_reduced
    win_col = ['win']
